...
let scopeData: Uint8Array | null = null;
// map to hold pending flash timeouts per channel
const _flashTimeouts: Map<number, ReturnType<typeof setTimeout>> = new Map();
...
function startUI(playbackStartTimeRef: PlaybackStartTimeGetter): void {
    if (uiRaf) {
...
function initScopeCanvas(): boolean {
...
function drawScope(): void {
    if (!scopeCtx || !analyserNode || !scopeCanvas || !scopeData) {
        return;
    }
    const rect = scopeCanvas.getBoundingClientRect();
    const w = rect.width;
    const h = rect.height;
    analyserNode.getByteTimeDomainData(scopeData as Uint8Array<ArrayBuffer>);
...
function updatePlayhead(playbackStartTimeRef: PlaybackStartTimeGetter): void {
...
function renderMixer(): void {
    ...
    tracks.forEach((t, i) => {
        ...
        mute.title = 'Mute';
        mute.textContent = 'M';
        mute.addEventListener('click', (e: MouseEvent) => {
            e.stopPropagation();
            t.muted = !t.muted;
            updateMixerGains();
            updateMixerUI();
        ...
        solo.title = 'Solo';
        solo.textContent = 'S';
        solo.addEventListener('click', (e: MouseEvent) => {
            e.stopPropagation();
            t.solo = !t.solo;
            updateMixerGains();
            updateMixerUI();
        ...
        const fader = document.createElement('input') as HTMLInputElement;
        fader.type = 'range';
        fader.min = '0';
        fader.max = '1';
        fader.step = '0.01';
        ...
        ch.addEventListener('click', (e: MouseEvent) => {
            ...
            if (
                target &&
                (target.classList.contains('mute') || target.classList.contains('solo'))
            ) {
                return;
            }
...
function flashChannel(index: number, ms = 160): void {
...
function renderFilterUI(trackIndex: number): void {
    ...
    const freq = document.createElement('input') as HTMLInputElement;
    freq.type = 'range';
    freq.min = '0';
    freq.max = '1000';
    freq.step = '1';
    ...
    const freqToSlider = (f: number): number => {
        const logMin = Math.log10(FREQ_MIN);
        const logMax = Math.log10(FREQ_MAX);
        const logF = Math.log10(f);
        return ((logF - logMin) / (logMax - logMin)) * 1000;
    };
    const sliderToFreq = (s: number): number => {
        const logMin = Math.log10(FREQ_MIN);
        const logMax = Math.log10(FREQ_MAX);
        const t = s / 1000;
    ...
    const q = document.createElement('input') as HTMLInputElement;
    q.type = 'range';
    q.min = '0';
    q.max = '1000';
    q.step = '1';
    ...
    const qToSlider = (qVal: number): number => {
    ...
    const sliderToQ = (s: number): number => {
        const logMin = Math.log10(Q_MIN);
        const logMax = Math.log10(Q_MAX);
        const t = s / 1000;
        return 10 ** (logMin + t * (logMax - logMin));
...
function updateMixerUI(): void {
    ...
    grid.querySelectorAll('.channel').forEach((el) => {
        ...
        const faderEl = el.querySelector('.fader-wrap input') as HTMLInputElement | null;
        if (faderEl) {
            const val =
